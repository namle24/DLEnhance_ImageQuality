...
class SFT_Layer(nn.Module):
    def __init__(self, drct_channels=180, mae_channels=384):
        super().__init__()
        self.scale_conv = nn.Conv2d(mae_channels, drct_channels, 1)
        self.shift_conv = nn.Conv2d(mae_channels, drct_channels, 1)
    
    def forward(self, drct_feat, mae_feat):
        mae_resized = F.interpolate(mae_feat,
                                    size=drct_feat.shape[-2:],
                                    mode='bilinear',
                                    align_corners=False)
        scale = self.scale_conv(mae_resized)
        shift = self.shift_conv(mae_resized)
        return drct_feat * (1 + scale) + shift
    ...
    def __init__(self, checkpoint_path=None, img_size=256, patch_size=8):
        ...
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            if 'model_state_dict' in ckpt:
                ...
                self.mae.load_state_dict(ckpt['params'])
            else:
...
class DRCT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_sft:
            self.mae_wrapper = PD_MAE_Encoder_Wrapper(checkpoint_path=mae_checkpoint, img_size=256)
            sft_indices = [1, 3, 5] if len(depths) >= 6 else [1, 3]
            self.sft_layers = nn.ModuleDict({
                str(i): SFT_Layer(drct_channels=embed_dim, mae_channels=384)
                for i in sft_indices
            })
    ...
    def forward_features(self, x, mae_feat=None):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x, x_size)
            if self.use_sft and str(i) in self.sft_layers and mae_feat is not None:
                # 1. Un-embed to 2D
                x_2d = self.patch_unembed(x, x_size)  # [B, C, H, W]
                # 2. Apply SFT
                x_2d = self.sft_layers[str(i)](x_2d, mae_feat)
                # 3. Embed back to 1D sequence
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, mae_feat)) + x
